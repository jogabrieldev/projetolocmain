...
export const LocacaoModel = {
  ...
  try {
    ...
    // Inserir locação
    const insertLocacao = `
      INSERT INTO clieloc(cllonmlo, clloidcl, cllodtdv, cllodtlo, cllopgmt,
        cllorua, cllocep, clloclno, cllobair, cllocida, cllorefe,
        clloqdlt, clloresi, cllocpcn, cllodesc)
    ...
    // Inserir bens vinculados
    const insertBens = `
      INSERT INTO bensloc (belocodb, belobem, belodtin, belodtfi, beloqntd,
        beloobsv, belostat, beloidcl, belocontr)
    ...
    for (const ben of bens) {
      const valores = [
        ben.codeBen, ben.produto, ben.dataInicio, ben.dataFim, ben.quantidade,
        ben.observacao, ben.status, clloid, ben.contrato?.trim() || null
      ];
