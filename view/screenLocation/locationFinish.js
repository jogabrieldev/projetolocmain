...
function preencherFormularioDeEdicao(locacao) {
  ...
  const statusDaLocacao = locacao.bens.length
    ? locacao.bens[0].belostat.trim()
    : "";
  document.getElementById("statEdit").value = statusDaLocacao;
...
function editarlocationFinish(id) {
  ...
  if (buttonSave) {
    buttonSave.addEventListener("click", async (event) => {
      ...
      console.log("locaçaõ", locacaoEncontrada);
      ...
      for (let i = 1; i <= 5; i++) {
        ...
        const dataInicioStr = document.getElementById(
          `dataInicio${i}Edit`
        ).value;
        ...
        if (
          ...
          console.warn(`⚠️ Campos ausentes no item ${i}, será ignorado.`);
          continue;
        }

        if (!isDataValida(dataInicioStr) || !isDataValida(dataFimStr)) {
          Toastify({
            text: `Item ${i}: data de início ou fim INVALIDA. veriique por favor`,
            duration: 3000,
            close: true,
            gravity: "top",
            position: "center",
      ...
      if (dataInicio < hoje || dataInicio > hoje) {
        ...
        if (dataFim <= dataInicio) {
          Toastify({
            text: `Item ${i}: a data de FIM deve ser maior que a data de INÍCIO.`,
            duration: 3000,
            close: true,
            gravity: "top",
            position: "center",
        ...
        const bem = {
          ...
          belodtfi: dataFimStr,
          belostat: document.getElementById("statEdit").value,
        };
      ...
      let novosBensInseridos = false;
      let bensForamAtualizados = false;

      // === INSERÇÃO DE NOVOS BENS ===
      if (bensNovos.length > 0) {
        console.log("Novos bens", bensNovos);
      ...
      try {
        ...
        // Exibir toasts conforme o que aconteceu:
        if (novosBensInseridos) {
          Toastify({
        ...
        if (!novosBensInseridos && !bensForamAtualizados) {
          Toastify({
        ...
        Toastify({
          text: "Erro ao atualizar no server.",
          duration: 3000,
          close: true,
          gravity: "top",
          position: "center",
